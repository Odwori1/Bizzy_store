#!/bin/bash
echo "🚀 Fixing logo component to support all variants..."

cd ~/Bizzy_store/frontend

# Backup the original Logo component
cp src/components/Logo.tsx src/components/Logo.tsx.backup

# Create the fixed Logo component
cat > src/components/Logo.tsx << 'EOF'
import React from 'react';

interface LogoProps {
  size?: number;
  variant?: 'shield-eye' | 'minimal' | 'badge' | 'third-eye' | 'simple' | 'eye' | 'abstract';
  className?: string;
  showText?: boolean;
  monochrome?: boolean;
}
...
export const BizzyLogo: React.FC<LogoProps> = ({
  ...
  // Third Eye Variant - Main Concept from LogoTest
  if (variant === 'third-eye') {
    return (
      <div className="flex items-center gap-4">
        <svg
          width={size}
          height={size}
          viewBox="0 0 60 60"
          className={`bizzy-logo-third-eye ${className}`}
        >
          <circle cx="30" cy="30" r="25" fill={monochrome ? "#1F2937" : "#1E40AF"}/>
          <circle cx="30" cy="30" r="18" fill="white"/>
          <circle cx="30" cy="30" r="12" fill="#3B82F6"/>
          <circle cx="30" cy="30" r="6" fill="#0F172A"/>
          {/* Third eye symbol */}
          <circle cx="30" cy="15" r="4" fill="#10B981"/>
        </svg>
    ...
    return (
      <div className="flex items-center gap-3">
        <svg
          ...
          height={size}
          viewBox="0 0 60 60"
          className={`bizzy-logo-simple ${className}`}
        >
          <circle cx="30" cy="30" r="20" fill={monochrome ? "#1F2937" : "#1E40AF"}/>
        ...
        {showText && (
          <div className="flex flex-col">
            <span className="text-xl font-bold text-gray-900">Bizzy</span>
            <span className="text-xs text-gray-500 -mt-1">Simple</span>
          </div>
        )}
      </div>
    );
    ...
    return (
      <div className="flex items-center gap-3">
        <svg
          ...
          height={size}
          viewBox="0 0 60 60"
          className={`bizzy-logo-eye ${className}`}
        >
          <circle cx="30" cy="30" r="25" fill={monochrome ? "#1F2937" : "#3B82F6"}/>
        ...
        {showText && (
          <div className="flex flex-col">
            <span className="text-xl font-bold text-gray-900">Bizzy</span>
            <span className="text-xs text-gray-500 -mt-1">Eye</span>
          </div>
        )}
      </div>
    );
    ...
    return (
      <div className="flex items-center gap-3">
        <svg
          ...
          height={size}
          viewBox="0 0 60 60"
          className={`bizzy-logo-abstract ${className}`}
        >
          <rect x="10" y="10" width="40" height="40" rx="10" fill={monochrome ? "#1F2937" : "#8B5CF6"}/>
          <circle cx="30" cy="30" r="12" fill="#3B82F6"/>
          <path d="M20 20 L40 40 M40 20 L20 40" stroke="white" strokeWidth="3"/>
        </svg>

        {showText && (
          <div className="flex flex-col">
            <span className="text-xl font-bold text-gray-900">Bizzy</span>
            <span className="text-xs text-gray-500 -mt-1">Abstract</span>
          </div>
        )}
      </div>
    );
...
echo "✅ Logo component updated with all variants!"
EOF
